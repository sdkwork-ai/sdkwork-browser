...
`sdkwork-browser` ships the standard nine `bin/` entrypoints. Shared behavior
...
| Script | Purpose |
| --- | --- |
| `docker-image.sh` | build / push / save / load / update / inspect `registry.sdkwork.com/apps/sdkwork-browser-standalone:<version>` |
...
Declared app types: `server,desktop,flutter,h5,pc`. Default image tag comes from
